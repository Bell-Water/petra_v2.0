...
public class PlanService {
    ...
    public Response deletePlan(User user, Long planId) {

        UserIdDto userIdDto = planRepository.findPlanUserId(planId);
        if(userIdDto.getUserId().equals(user.getId())) {
            planRepository.deleteById(planId);
            return new Response();
        } else {
            throw new IllegalArgumentException("plan의 userId와 id가 일치하지 않습니다.");
        }
    }
}
